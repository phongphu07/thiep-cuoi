...
// The file has {theme.bgMain} everywhere because it was a copy of TraditionalTemplate.
// We should replace all `${theme.something}` with hardcoded Navy colors.
// bgMain -> bg-[#001f3f]
// bgLight -> bg-[#00152b]
// bgBank -> bg-[#001f3f]
// textMain -> text-[#d4af37]
// textAccent -> text-[#f8e5a5]
...
// Also remove `theme={theme}` and `theme: any` from props
...
// Now we replace the COVER SCREEN and ACTUAL PAGE CONTENT.
const coverStart = content.indexOf('{/* COVER SCREEN OVERLAY - CINEMATIC SPLIT */}');
const coverEnd = content.indexOf('{/* ACTUAL PAGE CONTENT */}', coverStart + 1);
...
content = content.substring(0, coverStart) + newCover + content.substring(coverEnd);

// Replace ACTUAL PAGE CONTENT top arch header with Hexagon
const pageStart = content.indexOf('{/* ACTUAL PAGE CONTENT */}');
...
content = content.substring(0, pageStart) + newPageHeader + content.substring(infoSection);

// Update Backgrounds
// All instances of bg-[#fdfaf6] (white background for sections) should become navy.
